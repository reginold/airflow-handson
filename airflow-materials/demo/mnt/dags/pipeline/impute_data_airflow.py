...
def _save_datasets(
    ...
    train_features.to_csv(str(out_train_features), index=False)
    test_features.to_csv(str(out_test_features), index=False)
    train_target.to_csv(str(out_train_target), index=False)
    test_target.to_csv(str(out_test_target), index=False)
...
def ImputeWithMostFrequent(
    train_df, test_df, cols=["country", "province", "region_1", "variety"]
):
...
def ImputeMissingValues(train_df, test_df):
    ...
    train_features = (
        interim_train_4.join(interim_train_3)
        .join(interim_train_2)
        .join(interim_train_1)
    )

    test_features = (
        interim_test_4.join(interim_test_3).join(interim_test_2).join(interim_test_1)
    )
...
def impute_datasets(in_train_csv, in_test_csv, out_dir):
    ...
    imputed_data = ImputeMissingValues(train_df, test_df)
